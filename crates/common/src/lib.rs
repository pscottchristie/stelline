...
// ─────────────────────────────────────────────────────────────────────────────
// AdminEntity / ZoneEntitySnapshot — per-entity data for the admin world view
// ─────────────────────────────────────────────────────────────────────────────

/// Lightweight entity data for the admin world view canvas.
///
/// Carries just enough information to render a dot on the admin dashboard's
/// top-down map: position, kind, and a human-readable label (username for
/// players, empty for everything else).
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
...
/// Per-zone entity snapshot for the admin canvas.
///
/// Emitted by each zone at a reduced rate (e.g. every 4th tick = 5 Hz) and
/// aggregated by the WorldCoordinator into [`AdminSnapshot`].
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct ZoneEntitySnapshot {
    pub zone_id: ZoneId,
    pub aoi_radius: f32,
    pub width: f32,
    pub height: f32,
    pub entities: Vec<AdminEntity>,
}
...
pub struct AdminSnapshot {
    ...
    /// Per-zone entity snapshots for the admin world view canvas.
    /// Updated at a lower rate than telemetry (~5 Hz).
    pub entity_snapshots: Vec<ZoneEntitySnapshot>,
}
...
/// A unique identifier for a persistent player character.
///
/// Backed by a UUID string (generated by the database). Unlike [`EntityId`]
/// which is ephemeral and assigned per-session, a `CharacterId` survives
/// across logins.
...
/// Player character race. Stored as SMALLINT in the database.
///
/// Minimal set for Phase 2 — extend later as races are designed.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[repr(u8)]
pub enum Race {
    Human = 0,
...
impl Race {
    /// Convert from a raw `u8` value (e.g. from the database).
    ...
    /// Convert to a raw `u8` for database storage.
...
/// Player character class. Stored as SMALLINT in the database.
///
/// Minimal set for Phase 2 — extend later as classes are designed.
#[derive(Debug, Clone, Copy, PartialEq, Eq, Hash, Serialize, Deserialize)]
#[repr(u8)]
pub enum Class {
    Warrior = 0,
...
/// Character data returned in list/create responses — what the client sees
...
/// Full character data including world position — used by the game server
/// to place the player in the correct zone at the correct position.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
pub struct CharacterFullInfo {
    pub id: CharacterId,
    pub name: String,
    pub race: Race,
    pub class: Class,
...
mod tests {
    ...
    #[test]
    fn character_id_new_and_get() {
        let id = CharacterId::new("abc-123".to_string());
        assert_eq!(id.get(), "abc-123");
    }

    #[test]
    fn character_id_equality() {
        let a = CharacterId::new("same".to_string());
        let b = CharacterId::new("same".to_string());
        let c = CharacterId::new("diff".to_string());
        assert_eq!(a, b);
        assert_ne!(a, c);
    }

    #[test]
    fn character_id_usable_as_hashmap_key() {
        use std::collections::HashMap;
        let mut map: HashMap<CharacterId, &str> = HashMap::new();
        map.insert(CharacterId::new("id-1".to_string()), "Arthas");
        assert_eq!(map[&CharacterId::new("id-1".to_string())], "Arthas");
    }

    #[test]
    fn character_id_display() {
        let s = format!("{}", CharacterId::new("uuid-here".to_string()));
        assert!(s.contains("uuid-here"));
    }

    #[test]
    fn character_id_serde_round_trip() {
        let original = CharacterId::new("test-uuid".to_string());
    ...
    #[test]
    fn race_from_u8_valid() {
        assert_eq!(Race::from_u8(0), Some(Race::Human));
        assert_eq!(Race::from_u8(1), Some(Race::Orc));
        assert_eq!(Race::from_u8(2), Some(Race::Dwarf));
        assert_eq!(Race::from_u8(3), Some(Race::Elf));
    }

    #[test]
    fn race_from_u8_invalid() {
        assert_eq!(Race::from_u8(4), None);
        assert_eq!(Race::from_u8(255), None);
    }

    #[test]
    fn race_to_u8_round_trip() {
        for v in 0..=3u8 {
            let race = Race::from_u8(v).unwrap();
            assert_eq!(race.to_u8(), v);
        }
    }
    ...
    fn race_display() {
        ...
        assert_eq!(format!("{}", Race::Elf), "Elf");
    }

    #[test]
    fn race_serde_round_trip() {
        let original = Race::Dwarf;
        let json = serde_json::to_string(&original).unwrap();
        let restored: Race = serde_json::from_str(&json).unwrap();
        assert_eq!(original, restored);
    }
    ...
    #[test]
    fn class_from_u8_valid() {
        assert_eq!(Class::from_u8(0), Some(Class::Warrior));
        assert_eq!(Class::from_u8(1), Some(Class::Mage));
        assert_eq!(Class::from_u8(2), Some(Class::Priest));
        assert_eq!(Class::from_u8(3), Some(Class::Rogue));
    }

    #[test]
    fn class_from_u8_invalid() {
        assert_eq!(Class::from_u8(4), None);
        assert_eq!(Class::from_u8(255), None);
    }

    #[test]
    fn class_to_u8_round_trip() {
        for v in 0..=3u8 {
            let class = Class::from_u8(v).unwrap();
    ...
    #[test]
    fn class_display() {
        assert_eq!(format!("{}", Class::Warrior), "Warrior");
        assert_eq!(format!("{}", Class::Rogue), "Rogue");
    }

    #[test]
    fn class_serde_round_trip() {
        let original = Class::Mage;
        let json = serde_json::to_string(&original).unwrap();
    ...
    #[test]
    fn character_info_serde_round_trip() {
        let original = CharacterInfo {
            id: CharacterId::new("char-1".to_string()),
        ...
        let json = serde_json::to_string(&original).unwrap();
        let restored: CharacterInfo = serde_json::from_str(&json).unwrap();
        assert_eq!(original, restored);
    }

    #[test]
    fn character_full_info_serde_round_trip() {
        let original = CharacterFullInfo {
            id: CharacterId::new("char-2".to_string()),
        ...
        let json = serde_json::to_string(&original).unwrap();
        let restored: CharacterFullInfo = serde_json::from_str(&json).unwrap();
        assert_eq!(original, restored);
    }

    #[test]
    fn character_full_info_contains_position() {
        let info = CharacterFullInfo {
            id: CharacterId::new("x".to_string()),
            name: "Test".to_string(),
            race: Race::Orc,
            class: Class::Rogue,
            level: 1,
            zone_id: ZoneId::new(3),
            position: Vec3::new(50.0, 10.0, 75.0),
        };
        assert_eq!(info.position, Vec3::new(50.0, 10.0, 75.0));
        assert_eq!(info.zone_id, ZoneId::new(3));
    }
